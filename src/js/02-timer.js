    ...
    if (selectedDates[0].getTime() - currentDate.getTime() < 0) {
      ...
      startBtn.addEventListener('click', () => {
        const timerId = setInterval(() => {
          const currentTime = new Date();
          const ms = selectedDates[0].getTime() - currentTime.getTime();
          const formattedTime = convertMs(ms);
          timerDays.textContent = formatTwoDigits(formattedTime.days);
          timerHours.textContent = formatTwoDigits(formattedTime.hours);
          timerMinutes.textContent = formatTwoDigits(formattedTime.minutes);
          timerSeconds.textContent = formatTwoDigits(formattedTime.seconds);
          if (ms < 1000) {
            clearInterval(timerId);
            timerDays.textContent = '00';
            timerHours.textContent = '00';
